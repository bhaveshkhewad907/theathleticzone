...
export default function LeaveReview({ onClose, onSuccess }: LeaveReviewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 🚀 ARCHITECTURE FIX: 'sport' is officially purged from the payload
      await api.post("/reviews", { rating, content });
      toast.success("Review submitted! It is now live on the Landing Page.");
      setContent("");
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h3 className="text-xl font-black italic uppercase tracking-tighter text-white mb-1">
              Platform <span className="text-amber-500">Review</span>
            </h3>
            <p className="text-[#8A94A6] text-[10px] font-bold uppercase tracking-widest">
              Share your experience. Your review will be public on the homepage.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? "Transmitting..." : "Submit Public Review"}
