...
export default function AthleteDashboard() {
  ...
  const getSportDisplayName = () => {
    const userSport = auth?.user?.sports?.[0] || profile?.sport;
    if (!userSport) return "Unassigned";
    const sportId =
      typeof userSport === "object" && "_id" in userSport
        ? userSport._id
        : userSport;
    const found = sportsRegistry.find((s) => String(s._id) === String(sportId));
                ...
                {activeCourse ? (
                  <div className="flex flex-col h-full relative">
                    ...
                    <div className="p-6 flex flex-col flex-1 relative z-10">
                      ...
                      <button
                        ...
                        className="mt-auto w-full py-5 rounded-[12px] bg-amber-500 text-black font-black text-xs uppercase tracking-[0.2em] hover:bg-amber-400 transition-all shadow-[0_10px_20px_rgba(245,158,11,0.2)] active:scale-95 flex justify-center items-center gap-2"
                      >
                        <PlayCircle size={18} />
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-10 text-center border-2 border-dashed border-white/5 rounded-[12px] bg-black/20 m-6">
                    ...
                    <p className="text-xs text-[#8A94A6] max-w-xs">
                      You have not been assigned a program yet. If you just
                      completed your assessment, please wait for the algorithm
